...
contract TestBurnNonFungible {
    ...
    function testWhenCreditIsFungible() external {
        ...
        proxyCredit.burnNonFungible(contractType, testAccounts[0]);
        (result, ) = throwProxy.execute();
        Assert.isFalse(
            result,
            "should not pass since the type of credit is fungible"
        );
    }
    ...
    function testWhenCallerHasNoPermission() external {
        ...
        proxyCredit.burnNonFungible(contractType | 1, testAccounts[0]);
        (result, ) = throwProxy.execute();
        Assert.isFalse(
            result,
    ...
    function testWhenBurnedCreditIsNotItemID() external {
        ...
        Assert.isFalse(
            result,
            "should not pass since the burned credit is not item id"
        );
    }
    ...
    function testWhenSuccess() external {
        ...
        Assert.equal(
            credit.balanceOf(testAccounts[0], contractType),
            0,
